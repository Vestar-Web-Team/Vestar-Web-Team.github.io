...
import { createSignal, useObserver, withSolid } from 'react-solid-state';
...
const SlideBox = withSolid((props: {delay: number}) => {
    const slideboxItem = {
        default: "",
        data: [
    ...
    const [manualswitch, manualSwitch] = createSignal<number>(-1);
    ...
    function useSlider(N: number, mns: number, speed = 4000) {
        const [slider, setSlider] = createSignal<number>(0);
        const [slideswitch, setSlideSwitch] = createSignal<boolean>(true);

        if (mns === -1) {
            useIntervalFn(() => {
                if (slideswitch) {
                    setSlider(() => (Math.floor((props.delay / speed))) % N)
                }
            }, 4000)
