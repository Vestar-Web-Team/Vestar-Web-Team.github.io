...
export default function Fullpage(props: { page?: number; children?: JSXElement; }) {
  ...
  const translateY = createMemo(() => heights().slice(0, page() - 1).join((' - ')) || '0px');
  createEffect(() => {
    transformRef.setAttribute('style', `transform: translateY(calc(0px - ${translateY()}))`);
  });
  ...
  onMount(() => {
    ...
    useEventListener(containerRef, 'pointerdown', (ev: PointerEvent) => {
    ...
    useEventListener(containerRef, 'pointermove', (ev: PointerEvent) => {
      if (!dragging) return;
    ...
    useEventListener(containerRef, 'pointerup', (ev: PointerEvent) => {
    ...
    useEventListener(
      ...
      'wheel',
      debounce(
        (ev: WheelEvent) => ev.deltaY > 0 ? nextPage() : lastPage(),
        500,
        { maxWait: 1000, leading: true, trailing: false }
      ),
      {passive: true}
    );
  });
  ...
  createEffect(() => { console.log(page()); });

  return <div ref={containerRef} class='w-dvw h-dvh overflow-hidden z-50 pointer-events-auto'>
    <div ref={transformRef} class='h-auto transition-transform duration-500 ease-in-out'>
      {resolved()}
    </div>
  </div>;
}
...
export function Page(props: { height?: string; children?: JSXElement; }) {
  ...
  return <div data-fullpage="page" data-height={height()} class='relative w-auto h-auto' style={{ height: height() }}>{props.children}</div>;
}